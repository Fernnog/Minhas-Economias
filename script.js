...
// Algoritmo de renderização com Saldo Contínuo (Atualizado com data-label para CSS Mobile)
function renderTransactions() {
    list.innerHTML = '';
    
    // Passo 1: Ordena cronologicamente (antigo -> novo) para calcular a matemática do saldo exato
    ...
    // Passo 2: Reverte (novo -> antigo) para exibir as movimentações mais recentes no topo
    const sortedForDisplay = transactionsWithBalance.sort((a, b) => new Date(b.date) - new Date(a.date));

    // Ícones minimalistas em SVG para padronização visual
    ...
    sortedForDisplay.forEach(trans => {
        ...
        const saldoClass = trans.saldo < 0 ? 'despesa' : ''; // Destaca a fonte em vermelho se o saldo estiver negativo

        // A inclusão de data-label permite que o CSS transforme a tabela em cards no mobile
        tr.innerHTML = `
            <td data-label="Data">${formattedDate}</td>
            <td data-label="Descrição">${trans.desc}${trans.isRecurring ? ' 🔄' : ''}</td>
            <td data-label="Categoria">${trans.category}</td>
            <td data-label="Tipo" class="${cssClass}">${trans.type.charAt(0).toUpperCase() + trans.type.slice(1)}</td>
            <td data-label="Valor" class="${cssClass}">${formattedAmount}</td>
            <td data-label="Saldo" class="${saldoClass} font-weight-bold">${formattedSaldo}</td>
