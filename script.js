...
function renderSettingsForm() {
    const listContainer = document.getElementById('pinned-categories-list');
    if (!listContainer) return;

    const rawBudgets = JSON.parse(localStorage.getItem('fin_budgets')) || [];
    const budgetCategories = [...new Set(rawBudgets.map(b => b.category))];
    
    listContainer.innerHTML = budgetCategories.map(cat => `
        <div class="checkbox-group" style="margin-bottom: 0.8rem; display: flex; align-items: center; gap: 8px;">
            <input type="checkbox" id="pin-${cat}" value="${cat}" ${pinnedBudgets.includes(cat) ? 'checked' : ''}>
            <label for="pin-${cat}">${cat}</label>
        </div>
    `).join('') || '<p style="font-size: 0.85rem; color: var(--text-light);">Crie orçamentos primeiro na seção correspondente para fixá-los aqui.</p>';
}
...
function renderPinnedBudgets(gastosDoMes) {
    ...
    if (pinnedBudgets.length === 0) {
        container.innerHTML = '<p style="color: var(--text-light); font-size: 0.9rem; padding: 1rem;">Nenhum orçamento fixado. Use o botão Painel para configurar.</p>';
        return;
    }
