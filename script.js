...
function notifyCategoryChange() {
    ...
    // Notifica o módulo de orçamento se ele estiver disponível
...
function init() {
    ...
    // === Listeners dos botões de navegação de mês ===
...
function updateDashboardData() {
    ...
    // Calcula o mês sendo visualizado com base no offset
    ...
    // Atualiza o label de navegação e o estado dos botões
    ...
    if (btnNext) btnNext.disabled = dashboardMonthOffset >= 1; // limita a 1 mês à frente

    // Mostra/oculta o card de Saldo Atual conforme o mês
    ...
    // Atualiza label do card de projeção
    ...
    todasTransacoes.forEach(trans => {
        ...
        // Saldo Atual: apenas transações até hoje E somente no mês corrente
        if (isMesCorrente && dataTrans <= hoje) {
            if (trans.type === 'receita') saldoAtualTotal += trans.amount;
            else saldoAtualTotal -= trans.amount;
        }

        // Saldo fim do mês: todas as transações do mês visualizado
...
function updateCategorySelect() {
    ...
    // Mantém a lista limpa e a categoria de fallback no fim
...
// === CATEGORY FORM HANDLER (PRIORITY 1) ===
...
function renderPinnedBudgets(gastosDoMes, mesAtual, anoAtual) {
    ...
    // Suporte ao chamado legado sem parâmetros de mês
