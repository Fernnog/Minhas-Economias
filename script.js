...
form.addEventListener('submit', function(e) {
    ...
    if (id) {
        ...
        if (isParcelada) {
            ...
            // Lógica de exceção de recorrência / Reajuste
            if (exceptionParent) {
                ...
                if (parentTx) {
                    if (editScope === 'this_and_future') {
                        // Lógica 1: Atualizar a partir de agora (Reajuste)
                        // Encerra a recorrência anterior na data da exceção
                        parentTx.recurrenceEndDate = exceptionDate;
                        newItemsToSync.push(parentTx);

                        // Cria a nova recorrência com o novo valor
                        const transactionData = { id: Date.now().toString(), type, amount, category, date, desc, isRecurring: true };
                        transactions.push(transactionData);
                        newItemsToSync.push(transactionData);
                    } else {
                        // Lógica 2: Exceção Isolada (Apenas este mês)
    ...
    // Garante que o seletor de escopo volte a ficar oculto
...
// Nova funcionalidade: Exceção Isolada de Recorrência / Reajuste
window.editSingleProjected = function(projId, dateStr) {
    ...
    if (trans) {
        ...
        // NOVO: Exibir o seletor de escopo de edição
        const scopeContainer = document.getElementById('edit-scope-container');
