...
function init() {
    ...
    document.getElementById('btn-next-month')?.addEventListener('click', () => {
        if (dashboardMonthOffset < 2) { dashboardMonthOffset++; updateDashboardData(); }
    });
...
/**
 * Navegação Rápida: Altera o DOM visualmente sem recalcular dados.
 * Atualizado com animação CSS forçada via Reflow (Synchronous Layout).
 */
...
function updateDashboardData() {
    ...
    if (btnNext) btnNext.disabled = dashboardMonthOffset >= 2;
