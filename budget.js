const BudgetModule = (function() {
    ...
    // Função nomeada e separada para o submit, permitindo removeEventListener
    ...
    function init() {
        ...
        // Remove listener anterior antes de adicionar, garantindo que sempre
        // exista apenas UM listener ativo — independente de quantas vezes
        // init() seja chamado (pelo DOMContentLoaded e pelo script.js)
    ...
    function save() {
        ...
        // Validação: impede salvar com valor inválido ou zerado
        if (isNaN(amt) || amt <= 0) {
        ...
        // Variável para rastrear exatamente qual item sincronizar com o Firebase
        let itemToSync;
        ...
        if (idField.value) {
            ...
            if (idx > -1) {
                ...
                itemToSync = budgetLimits[idx]; // item editado correto
            }
            idField.value = '';
            ...
            itemToSync = newBudget; // item recém-criado
        }

        localStorage.setItem('fin_budgets', JSON.stringify(budgetLimits));

        // Sincronização Firebase — envia sempre o item correto (criado ou editado)
    ...
    function render() {
        ...
        // Cálculo de gastos com suporte a lançamentos recorrentes projetados no mês
        ...
        transactions.forEach(t => {
            ...
            if (t.isRecurring && (tYear < currentYear || (tYear === currentYear && tMonth < currentMonth))) {
                // Respeita a data de término da recorrência, se houver
        ...
        // Filtra orçamentos ativos e descarta qualquer registro com amount inválido
        // como segunda camada de proteção contra dados corrompidos
    ...
    function loadFromStorage() {
        ...
        if (Array.isArray(rawBudgets)) {
            // Separa os corrompidos (amount nulo ou NaN) dos válidos
            ...
            // Se encontrou corrompidos, limpa localStorage e apaga do Firebase automaticamente
            if (corrompidos.length > 0) {
                console.warn(`[Budget] ${corrompidos.length} orçamento(s) corrompido(s) removido(s) automaticamente.`);
                localStorage.setItem('fin_budgets', JSON.stringify(budgetLimits));
                corrompidos.forEach(b => {
                    if (typeof FirebaseModule !== 'undefined') {
                        FirebaseModule.deleteData('budgets', b.id);
                    }
                });
            }
        } else {
